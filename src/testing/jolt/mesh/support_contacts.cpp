...
public:
   void PostStep() override {
      ...
      auto Check = [&](const char* pchName, JPH::Vec3 cNormal,
                       JPH::Vec3 cPoint, bool bDrive, bool bHorizontal = false) {
         ...
         if(bHorizontal && (std::abs(sVelocity.Linear.GetZ()) > 1.0e-6f ||
                            std::abs(sVelocity.Angular.GetX()) > 1.0e-6f ||
                            std::abs(sVelocity.Angular.GetY()) > 1.0e-6f)) {
            THROW_ARGOSEXCEPTION("Steep Spot contact has vertical motor motion");
         }
      };
      ...
      Check("lower leg on rough ground", JPH::Vec3(-0.9987f, 0, 0.05f), JPH::Vec3(0.53f, 0.25f, -0.21f), true, true);
